 ...
 */
public class TemperatureFragment extends Fragment implements OnChartValueSelectedListener{
    ...
    public void onResume() {
        ...
        minTemp = mHours[0].getTemperature();
        maxTemp = mHours[0].getTemperature();
        for (int i = 0; i < mHours.length - SUBSTRACT_FROM_HOURS; i++) {
            yVals.add(new Entry(mHours[i].getTemperature(), i));

            //Find the Minimum value
            if (mHours[i].getTemperature() < minTemp) {
                minTemp = mHours[i].getTemperature();
            }

            //Find Maximum value
            if (mHours[i].getTemperature() > maxTemp) {
                maxTemp = mHours[i].getTemperature();
            }
        }
        ...
        xl.setDrawGridLines(false); //Don't draw the vertical grid lines
        xl.setEnabled(true);
        ...
        leftAxis.setValueFormatter(new MyValueFormatter(" \u2109")); //Set Y label to Fahrenheit (°F)
        leftAxis.setAxisMaxValue(maxTemp + COMPENSATE_MIN_MAX);//Set maximum value for Y axis
        leftAxis.setAxisMinValue(minTemp - COMPENSATE_MIN_MAX); //Set minimum value for Y axis
        ...
        LineDataSet set1 = new LineDataSet(yVals, "Temperature"); //Line chart
