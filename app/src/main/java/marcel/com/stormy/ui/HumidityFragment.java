 ...
 */
public class HumidityFragment extends Fragment implements OnChartValueSelectedListener {
    ...
    public void onResume() {
        ...
        mChart.setDrawGridBackground(false); //Allow to customize the backgreound set to true
        ...
        minHumidity =mHours[0].getHumidity();
        maxHumidity =mHours[0].getHumidity();
        for (int i = 0; i < mHours.length-SUBSTRACT_FROM_HOURS; i++) {
            yVals.add(new Entry(mHours[i].getHumidity(), i));

            //Find the Minimum value
            if(mHours[i].getHumidity()< minHumidity){
                minHumidity =mHours[i].getHumidity();
            }

            //Find Maximum value
            if(mHours[i].getHumidity()> maxHumidity){
                maxHumidity =mHours[i].getHumidity();
            }
        }
        ...
        leftAxis.setValueFormatter(new MyValueFormatter("%")); //Set Y label to Percentage (%)
        leftAxis.setAxisMaxValue(maxHumidity + COMPENSATE_MIN_MAX);//Set maximum value for Y axis
        leftAxis.setAxisMinValue(minHumidity - COMPENSATE_MIN_MAX); //Set minimum value for Y axis
